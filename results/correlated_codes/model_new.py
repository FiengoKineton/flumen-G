...
class CausalFlowModel(nn.Module):
    ...
    def forward(self, x, rnn_input, deltas):
        ...
        h0 = torch.stack(h0.split(self.control_rnn_size, dim=1)) if not self.mode_rnn else h0
        z = z.unsqueeze(0).expand(self.control_rnn_depth, -1, -1)
        c0 = torch.zeros_like(z if self.mode_rnn else h0) """

        h0, c0, tau = self.structure_function(x, deltas)

        rnn_out_seq_packed, _ = self.u_rnn(rnn_input, (h0, c0), tau)     #if self.mode_rnn else self.u_rnn(rnn_input, (h0, c0))
        h, h_lens = torch.nn.utils.rnn.pad_packed_sequence(rnn_out_seq_packed, batch_first=True)

        h_shift = torch.roll(h, shifts=1, dims=1)   
        h_temp = h0[-1]         # z[-1] if self.mode_rnn else h0[-1]
        h_shift[:, 0, :] = h_temp
