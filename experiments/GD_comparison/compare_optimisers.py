...
def get_latest_files():
    """Find the latest dataset for each optimizer in GD_comparison."""
    
    # Define the folder relative to the `experiments` directory
    folder = os.path.join(os.path.dirname(__file__), "GD_comparison")
...
def plot_comparison(dataframes):
    ...
    for opt, df in dataframes.items():
        plt.plot(df["epoch"], df["train_loss"], label=f"{opt} - Train Loss")
    plt.xlabel("Epoch")
    ...
    plt.figure(figsize=(10, 5))
    for opt, df in dataframes.items():
        plt.plot(df["epoch"], df["val_loss"], label=f"{opt} - Val Loss")
    plt.xlabel("Epoch")
